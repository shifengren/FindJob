...
public class IntervalHeap<K extends Comparable<K>, T>
        implements MaxHeap<K, T>, MinHeap<K, T> {
    ...
    public void replaceMin(K key, T value) {
        ...
        if (size > 1) {
            ...
            if (keys[1].compareTo(key) < 0) { //keys[1] < key
                swap(0, 1);
            }
            siftDownMin(0);
        }
    ...
    public void replaceMax(K key, T value) {
        ...
        keys[1] = key;
        // Swap with pair if necessary
        if (key.compareTo(keys[0]) < 0) { //key < keys[0]
            swap(0, 1);
        }
        siftDownMax(1);
    }
    ...
    private void siftInsertedValueUp() {
        ...
        if (u == 0) {
            ...
            if (keys[u].compareTo(keys[u-1]) < 0) { //keys[u] < keys[u - 1] If less than it's pair
            ...
            if (keys[u].compareTo(keys[u-1]) < 0) { // keys[u] < keys[u - 1] If less than it's pair
                ...
                if (keys[u].compareTo(keys[p-1]) < 0) { //keys[u] < keys[p - 1] If smaller than smaller parent pair
                    // Swap into min-heap side
                    u = swap(u, p - 1);
                    siftUpMin(u);
                }
            } else {
                if (keys[u].compareTo(keys[p]) > 0) { // keys[u] > keys[p]  If larger that larger parent pair
            ...
            if (keys[u].compareTo(keys[p]) > 0) { //keys[u] > keys[p]  If larger that larger parent pair
                ...
                u = swap(u, p);
                siftUpMax(u);
            } else if (keys[u].compareTo(keys[p-1]) < 0) { //keys[u] < keys[p - 1] If smaller than smaller parent pair
                // Swap into min-heap side
                u = swap(u, p - 1);
                siftUpMin(u);
            }
    ...
    private void siftUpMin(int c) {
        ...
        for (int p = (c / 2 - 1) & ~1; p >= 0 && keys[c].compareTo(keys[p]) < 0; c = p, p = (c / 2 - 1) & ~1) { //keys[c] < keys[p]
            swap(c, p);
        }
    }
    ...
    private void siftDownMin(int p) {
        for (int c = p * 2 + 2; c < size; p = c, c = p * 2 + 2) {
            if (c + 2 < size && keys[c+2].compareTo(keys[c]) < 0) { // keys[c + 2] < keys[c]
                c += 2;
            }
            if (keys[c].compareTo(keys[p]) < 0) { //keys[c] < keys[p]
                swap(p, c);
                // Swap with pair if necessary
                if (c + 1 < size && keys[c+1].compareTo(keys[c]) < 0) { // keys[c + 1] < keys[c]
                    swap(c, c + 1);
                }
            } else {
                break;
    ...
    private void siftDownMax(int p) {
        for (int c = p * 2 + 1; c <= size; p = c, c = p * 2 + 1) {
            if (c == size) {
                ...
                if (keys[c-1].compareTo(keys[p]) > 0) { //keys[c - 1] > keys[p]
                    swap(p, c - 1);
                }
                break;
            } else if (c + 2 == size) {
                // If there is only room for a right child lower pair
                if (keys[c+1].compareTo(keys[c]) > 0) { //keys[c + 1] > keys[c]
                    if (keys[c+1].compareTo(keys[c]) > 0) { // keys[c + 1] > keys[p]
                        swap(p, c + 1);
                    }
                    break;
                }
                ...
                if (keys[c+2].compareTo(keys[c]) > 0) { // keys[c + 2] > keys[c]
                    c += 2;
                }
            }
            if (keys[c].compareTo(keys[p]) > 0) { //keys[c] > keys[p]
    ...
    private boolean validateHeap() {
        ...
        for (int i = 0; i < size - 1; i += 2) {
            if (keys[i].compareTo(keys[i+1]) > 0)  //keys[i] > keys[i + 1]
                return false;
        }
        // Validate within parent interval
        for (int i = 2; i < size; i++) {
            K maxParent = keys[(i / 2 - 1) | 1];
            K minParent = keys[(i / 2 - 1) & ~1];
            if (keys[i].compareTo(maxParent) >0 || keys[i].compareTo(minParent) < 0) //keys[i] > maxParent || keys[i] < minParent
                return false;
        }
        return true;
    }
